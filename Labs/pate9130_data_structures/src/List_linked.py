...
class List:
    ...
    def prepend(self, value):
        ...
        if self._front is None:
            n_node = _List_Node(value, None)
            self._rear = n_node
            self._front = n_node
        else:
            n_node = _List_Node(value, self._front)
            self._front = n_node
    ...
    def remove_many(self, key):
        ...
        """
        fronk = self._front
        while fronk is not None and fronk._value == key:
            _ = self.remove_front()
            fronk = self._front
        
        fronk = self._front
        prev = _List_Node(None, None)
        while fronk is not None:
            if fronk._value == key:
                prev._next = fronk._next
                fronk._next = None
                fronk = prev._next
                self._count -= 1
            else:
    ...
    def count(self, key):
        ...
        """
        number = 0
        _, current, count = self._linear_search(key)
        if count != -1:
            while current != None:
    ...
    def clean(self):
        ...
        prev = _List_Node(None, None)
        while fronk is not None:
            _ , temp, _ = self._linear_search(fronk._value)
            if fronk is not temp:
                prev._next = fronk._next
                fronk._next = None
                fronk = prev._next
                self._count -= 1
    ...
    def combine(self, source1, source2):
        ...
        while var1 is not None or var2 is not None:
            if swap:
                if var1 is not None:
                    n_node = _List_Node(var1._value, None)
                    var1 = var1._next
                else:
                    n_node = _List_Node(var2._value, None) 
                    var2 = var2._next
            else:
                if var2 is not None:
                    n_node = _List_Node(var2._value, None)
