...
class BST:
        ...
        if node is None:
            ...
            if node._left is None and node._right is None:
                ...
                if node._left._right is None:
                    ...
                    chg_n = self._delete_node_left(node._left)
                    chg_n._left = node._left
                
                chg_n._right = node._right
                node = chg_n
    ...
    def _delete_node_left(self, parent):
        ...
        if kid._right is None:
            chg_n = kid
            parent._right = kid._left
            
        else:
            chg_n = self._delete_node_left(kid)
    ...
    def _is_identical_aux(self, m_node, o_node):
        
        if m_node is None and o_node is None:
            identical = True
        elif m_node is not None and o_node is not None and m_node._value == o_node._value and m_node._height == o_node._height:
            identical = self._is_identical_aux(m_node._left, m_node._left) and self._is_identical_aux(o_node._right, o_node._right)
        else:
            identical = False
    ...
    def parent(self, key):
        ...
        while n_node is not None and found is False:

            if key < n_node._value:
                parent = n_node
                node = n_node._left
            elif key > n_node._value:
                parent = node
                node = node._right
            else:
                found = True
    ...
    def _parent_aux(self, n_node, key, papi):
        ...
        if n_node is None:
            ...
            value = self._parent_aux(n_node._left, key, n_node)
        elif key > n_node._value:

            value = self._parent_aux(n_node._right, key, n_node)
        elif papi is None:

            value = None
        else:

            value = deepcopy(papi._value)
        return value
    ...
    def _one_child_count_aux(self, node):
        ...
        if node is None:
            ...
            count = self._one_child_count_aux(node._right) + self._one_child_count_aux(node._left)
    ...
    def _node_counts_aux(self, n_node):
        if n_node is None:
            one = 0
            two = 0
            zero = 0
            
        elif n_node._left is None and n_node._right is None:
            one = 0
            two = 0
            zero = 1
            
        elif n_node._left is not None and n_node._right is None:
            zero, one, two = self._node_counts_aux(n_node._left)
            one += 1
            
        elif n_node._left is None and n_node._right is not None:
            zero, one, two = self._node_counts_aux(n_node._right)
            one += 1
            
        else:
            zero_l, one_l, two_l = self._node_counts_aux(n_node._left)
            zero_r, one_r, two_r = self._node_counts_aux(n_node._right)
            zero = zero_l + zero_r
            one = one_l + one_r
    ...
    def _is_balanced_aux(self, node):
        ...
        elif abs(self._node_height(node._left)) - self._node_height(node._right):
            balanced = False
